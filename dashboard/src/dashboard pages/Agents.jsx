...
const Agents = () => {
  ...
  const openModal = () => {
    ...
    const nextId = AgentsData.length + 1;
    setIsModalOpen(true);
  ...
  const handleSubmit = () => {
    // Add the new entry to AgentsData
    setAgentsData([...AgentsData, formData]);
    // Close the modal
    closeModal();
  };
  ...
  const handleSave = () => {
    // Save the edited data to the AgentsData array
    const updatedAgentsData = AgentsData.map((entry) => {
  ...
  useEffect(() => {
    ...
    const storedData = JSON.parse(localStorage.getItem("AgentsData")) || [];
    setAgentsData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever AgentsData changes
    localStorage.setItem("AgentsData", JSON.stringify(AgentsData));
  }, [AgentsData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from AgentsData
      const updatedAgentsData = AgentsData.filter(
  ...
  return (
    <section className="flex bg-customgrey mx-auto max-w-full dark:bg-[#141625] duration-300">
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Agents</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Agents </NavLink> - List
            </div>
          </div>
