...
const BusinessRenewal = () => {
  ...
  const [businessDataRenewal, setBusinessDataRenewal] = useState([
    // {
    //   id: 1,
    //   title: "1",
    //   price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the businessDataRenewal array
    const updatedbusinessDataRenewal = businessDataRenewal.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setBusinessDataRenewal(updatedbusinessDataRenewal);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("businessDataRenewal")) || [];
    setBusinessDataRenewal(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever businessDataRenewal changes
    localStorage.setItem(
      "businessDataRenewal",
      JSON.stringify(businessDataRenewal)
    );
  }, [businessDataRenewal]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from businessDataRenewal
      const updatedbusinessDataRenewal = businessDataRenewal.filter(
  ...
  let filteredData = [...businessDataRenewal];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(businessDataRenewal);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "businessDataRenewal");
    XLSX.writeFile(wb, "businessDataRenewal.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "businessDataRenewal",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "businessDataRenewalPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert businessDataRenewal to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(businessDataRenewal);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Business Renewal</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Business Renewal </NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(businessDataRenewal)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={businessDataRenewal}>CSV</CSVLink>
                </button>
                <button
              ...
              >
                <table className="min-w-full my-2 ">
                  ...
                  {businessDataRenewal.length === 0 ? (
                    <tbody className="h-6">
                      <tr className="text-center text-customblack text-base font-normal relative">
                        <td className="absolute right-0 left-0">No data available in table</td>
                      </tr>
                    </tbody>
                  ) : (
                    <tbody>
                          ...
                          <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                            <div className="flex items-center gap-1">
                              <button onClick={() => handleEdit(business)}>
                                {" "}
                                <BiEdit className="text-customicongrey" />
                              </button>{" "}
                              <button
                                onClick={() => setEntryToDelete(business)}
                              >
                                <AiOutlineDelete className="text-customicongrey" />
                              </button>{" "}
                            </div>
                          </td>
                        </tr>
