...
const Businesses = () => {
  const [BusinessesData, setBusinessesData] = useState([
    {
      id: 1,
      name: "Hasham Khan",
      businessname: "demo testing service",
  ...
  const openModal = () => {
    ...
    const nextId = BusinessesData.length + 1;
    setIsModalOpen(true);
  ...
  const handleSubmit = () => {
    // Add the new entry to BusinessesData
    setBusinessesData([...BusinessesData, formData]);
    // Close the modal
    closeModal();
  };
  ...
  const handleSave = () => {
    // Save the edited data to the BusinessesData array
    const updatedBusinessesData = BusinessesData.map((entry) => {
  ...
  useEffect(() => {
    ...
    const storedData = JSON.parse(localStorage.getItem("BusinessesData")) || [];
    setBusinessesData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever BusinessesData changes
    localStorage.setItem("BusinessesData", JSON.stringify(BusinessesData));
  }, [BusinessesData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from BusinessesData
      const updatedBusinessesData = BusinessesData.filter(
  ...
  let filteredData = [...BusinessesData];
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Businesses</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Businesses </NavLink> - List
            </div>
          </div>
                  ...
                  <tbody>
                    {currentItems.map((business) => (
                      <tr key={business.id}>
                        ...
                        <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                          <p className="text-[#01B81A] rounded-full bg-[#DBEFE1] w-max px-3 font-medium">
                            {" "}
                            {business.status}
                          </p>
                        </td>
                        <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="text-btnpurple font-normal text-sm border rounded-sm border-btnpurple py-1 px-4 hover:text-white hover:bg-btnpurple hover:transition-all"
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => setEntryToDelete(business)}
                              className="text-[#FF0F0F] font-normal text-sm border rounded-sm border-[#FF0F0F] py-1 px-4 hover:text-white hover:bg-[#FF0F0F] hover:transition-all"
                            >
                              Delete
                            </button>
                            <button className="text-[#FA8B0C] font-normal text-sm border rounded-sm border-[#FA8B0C] py-1 px-4 hover:text-white hover:bg-[#FA8B0C] hover:transition-all">
                              Disable User
                            </button>
                          </div>
                        </td>
