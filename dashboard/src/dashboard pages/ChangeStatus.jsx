...
const ChangeStatus = () => {
  ...
  const handleSave = () => {
    // Save the edited data to the ChangeStatusData array
    const updatedChangeStatusData = ChangeStatusData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setChangeStatusData(updatedChangeStatusData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("ChangeStatusData")) || [];
    setChangeStatusData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever ChangeStatusData changes
    localStorage.setItem(
      "ChangeStatusData",
      JSON.stringify(ChangeStatusData)
    );
  }, [ChangeStatusData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from ChangeStatusData
      const updatedChangeStatusData = ChangeStatusData.filter(
  ...
  let filteredData = [...ChangeStatusData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(ChangeStatusData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "ChangeStatusData");
    XLSX.writeFile(wb, "ChangeStatusData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "ChangeStatusData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "ChangeStatusDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert ChangeStatusData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(ChangeStatusData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Change Status</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Change Status</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(ChangeStatusData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={ChangeStatusData}>CSV</CSVLink>
                </button>
                <button
