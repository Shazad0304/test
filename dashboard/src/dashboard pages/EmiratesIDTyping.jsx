...
const EmiratesIDTyping = () => {
  const componentPDF = useRef();
  const [EmiratesIDTypingData, setEmiratesIDTypingData] = useState([
    // {
    //   id: 1,
    //   title: "Anwar key",
    //   price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the EmiratesIDTypingData array
    const updatedEmiratesIDTypingData = EmiratesIDTypingData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setEmiratesIDTypingData(updatedEmiratesIDTypingData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("EmiratesIDTypingData")) || [];
    setEmiratesIDTypingData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever EmiratesIDTypingData changes
    localStorage.setItem(
      "EmiratesIDTypingData",
      JSON.stringify(EmiratesIDTypingData)
    );
  }, [EmiratesIDTypingData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from EmiratesIDTypingData
      const updatedEmiratesIDTypingData = EmiratesIDTypingData.filter(
  ...
  let filteredData = [...EmiratesIDTypingData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(EmiratesIDTypingData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "EmiratesIDTypingData");
    XLSX.writeFile(wb, "EmiratesIDTypingData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "EmiratesIDTypingData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "EmiratesIDTypingDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert EmiratesIDTypingData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(EmiratesIDTypingData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Emirates ID Typing</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Emirates ID Typing</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(EmiratesIDTypingData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={EmiratesIDTypingData}>CSV</CSVLink>
                </button>
                <button
...
export default EmiratesIDTyping;
