...
const FinalContractSubmission = () => {
  const componentPDF = useRef();
  const [FinalContractSubmissionData, setFinalContractSubmissionData] = useState([
    {
      id: 1,
      title: "FCS",
      EmiratesID:"1994-7635-1945",
      price: 3000,
      users: "active",
  ...
  const handleSave = () => {
    // Save the edited data to the FinalContractSubmissionData array
    const updatedFinalContractSubmissionData = FinalContractSubmissionData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setFinalContractSubmissionData(updatedFinalContractSubmissionData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("FinalContractSubmissionData")) || [];
    setFinalContractSubmissionData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever FinalContractSubmissionData changes
    localStorage.setItem(
      "FinalContractSubmissionData",
      JSON.stringify(FinalContractSubmissionData)
    );
  }, [FinalContractSubmissionData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from FinalContractSubmissionData
      const updatedFinalContractSubmissionData = FinalContractSubmissionData.filter(
  ...
  let filteredData = [...FinalContractSubmissionData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(FinalContractSubmissionData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "FinalContractSubmissionData");
    XLSX.writeFile(wb, "FinalContractSubmissionData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "FinalContractSubmissionData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "FinalContractSubmissionDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert FinalContractSubmissionData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(FinalContractSubmissionData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Final Contract Submission</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Final Contract Submission</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            <div className="sm:flex justify-between items-center px-6 py-4">
              ...
              <div className="mt-3 sm:w-auto w-max">
                ...
                >
                  ...
                  Apply For Final Contract Submission
                </NavLink>
              </div>
            </div>
            <hr />
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(FinalContractSubmissionData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={FinalContractSubmissionData}>CSV</CSVLink>
                </button>
                <button
              ...
              >
                <table className="min-w-full my-2 ">
                  ...
                  {FinalContractSubmissionData.length === 0 ? (
                    <tbody className="h-6">
...
export default FinalContractSubmission;
