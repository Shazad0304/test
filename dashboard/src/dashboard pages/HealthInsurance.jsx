...
const HealthInsurance = () => {
  ...
  const handleSave = () => {
    // Save the edited data to the HealthInsuranceData array
    const updatedHealthInsuranceData = HealthInsuranceData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setHealthInsuranceData(updatedHealthInsuranceData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("HealthInsuranceData")) || [];
    setHealthInsuranceData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever HealthInsuranceData changes
  ...
  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from HealthInsuranceData
      const updatedHealthInsuranceData = HealthInsuranceData.filter(
  ...
  let filteredData = [...HealthInsuranceData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(HealthInsuranceData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "HealthInsuranceData");
    XLSX.writeFile(wb, "HealthInsuranceData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "HealthInsuranceData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "HealthInsuranceDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert HealthInsuranceData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(HealthInsuranceData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Health Insurance</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Health Insurance</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(HealthInsuranceData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={HealthInsuranceData}>CSV</CSVLink>
                </button>
                <button
