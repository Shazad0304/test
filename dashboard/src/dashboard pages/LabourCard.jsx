...
const LabourCard = () => {
  ...
  const handleSave = () => {
    // Save the edited data to the LabourCardData array
    const updatedLabourCardData = LabourCardData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setLabourCardData(updatedLabourCardData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("LabourCardData")) || [];
    setLabourCardData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever LabourCardData changes
    localStorage.setItem(
      "LabourCardData",
      JSON.stringify(LabourCardData)
    );
  }, [LabourCardData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from LabourCardData
      const updatedLabourCardData = LabourCardData.filter(
  ...
  let filteredData = [...LabourCardData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(LabourCardData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "LabourCardData");
    XLSX.writeFile(wb, "LabourCardData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourCardData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourCardDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert LabourCardData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(LabourCardData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Labour Card</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Labour Card </NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(LabourCardData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={LabourCardData}>CSV</CSVLink>
                </button>
                <button
                        ...
                        <tr key={business.id}>
                          ...
                          <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                            <div className="flex items-center gap-1">
                              ...
                              >
                                <AiOutlineDelete className="text-customicongrey text-2xl rounded-full hover:text-btnpurple hover:bg-customgrey p-1" />
                              </button>{" "}
                            </div>
                          </td>
                        </tr>
