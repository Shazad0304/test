...
const LabourFee = () => {
  ...
  const [LabourFeeData, setLabourFeeData] = useState([
    // {
    //   id: 1,
    //   title: "Anwar key",
    //   price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the LabourFeeData array
    const updatedLabourFeeData = LabourFeeData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setLabourFeeData(updatedLabourFeeData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("LabourFeeData")) || [];
    setLabourFeeData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever LabourFeeData changes
    localStorage.setItem(
      "LabourFeeData",
      JSON.stringify(LabourFeeData)
    );
  }, [LabourFeeData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from LabourFeeData
      const updatedLabourFeeData = LabourFeeData.filter(
  ...
  let filteredData = [...LabourFeeData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(LabourFeeData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "LabourFeeData");
    XLSX.writeFile(wb, "LabourFeeData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourFeeData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourFeeDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert LabourFeeData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(LabourFeeData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Labour Fee</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Labour Fee</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(LabourFeeData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={LabourFeeData}>CSV</CSVLink>
                </button>
                <button
