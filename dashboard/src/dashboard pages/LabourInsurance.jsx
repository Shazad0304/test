...
const LabourInsurance = () => {
  ...
  const [LabourInsuranceData, setLabourInsuranceData] = useState([
    {
      id: 1,
      title: "Anwar key",
      price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the LabourInsuranceData array
    const updatedLabourInsuranceData = LabourInsuranceData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setLabourInsuranceData(updatedLabourInsuranceData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("LabourInsuranceData")) || [];
    setLabourInsuranceData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever LabourInsuranceData changes
    localStorage.setItem(
      "LabourInsuranceData",
      JSON.stringify(LabourInsuranceData)
    );
  }, [LabourInsuranceData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from LabourInsuranceData
      const updatedLabourInsuranceData = LabourInsuranceData.filter(
  ...
  let filteredData = [...LabourInsuranceData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(LabourInsuranceData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "LabourInsuranceData");
    XLSX.writeFile(wb, "LabourInsuranceData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourInsuranceData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "LabourInsuranceDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert LabourInsuranceData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(LabourInsuranceData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Labour Insurance</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Labour Insurance</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(LabourInsuranceData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={LabourInsuranceData}>CSV</CSVLink>
                </button>
                <button
...
export default LabourInsurance;
