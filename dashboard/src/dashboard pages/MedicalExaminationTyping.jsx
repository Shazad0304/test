...
const MedicalExaminationTyping = () => {
  const componentPDF = useRef();
  const [MedicalExaminationTypingData, setMedicalExaminationTypingData] = useState([
    // {
    //   id: 1,
    //   title: "Anwar key",
    //   price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the MedicalExaminationTypingData array
    const updatedMedicalExaminationTypingData = MedicalExaminationTypingData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setMedicalExaminationTypingData(updatedMedicalExaminationTypingData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("MedicalExaminationTypingData")) || [];
    setMedicalExaminationTypingData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever MedicalExaminationTypingData changes
    localStorage.setItem(
      "MedicalExaminationTypingData",
      JSON.stringify(MedicalExaminationTypingData)
    );
  }, [MedicalExaminationTypingData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from MedicalExaminationTypingData
      const updatedMedicalExaminationTypingData = MedicalExaminationTypingData.filter(
  ...
  let filteredData = [...MedicalExaminationTypingData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(MedicalExaminationTypingData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "MedicalExaminationTypingData");
    XLSX.writeFile(wb, "MedicalExaminationTypingData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "MedicalExaminationTypingData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "MedicalExaminationTypingDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert MedicalExaminationTypingData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(MedicalExaminationTypingData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Medical Examination Typing</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Medical Examination Typing</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            <div className="sm:flex justify-between items-center px-6 py-4">
              ...
              <div className="mt-3 sm:w-auto w-max">
                ...
                >
                  ...
                  Apply For Medical Examination Typing
                </NavLink>
              </div>
            </div>
            <hr />
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(MedicalExaminationTypingData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={MedicalExaminationTypingData}>CSV</CSVLink>
                </button>
                <button
              ...
              >
                <table className="min-w-full my-2 ">
                  ...
                  {MedicalExaminationTypingData.length === 0 ? (
                    <tbody className="h-6">
...
export default MedicalExaminationTyping;
