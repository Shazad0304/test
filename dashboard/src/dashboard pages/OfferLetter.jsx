...
const OfferLetter = () => {
  ...
  const handleSave = () => {
    // Save the edited data to the OfferLetterData array
    const updatedOfferLetterData = OfferLetterData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setOfferLetterData(updatedOfferLetterData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("OfferLetterData")) || [];
    setOfferLetterData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever OfferLetterData changes
    localStorage.setItem("OfferLetterData", JSON.stringify(OfferLetterData));
  }, [OfferLetterData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from OfferLetterData
      const updatedOfferLetterData = OfferLetterData.filter(
  ...
  let filteredData = [...OfferLetterData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(OfferLetterData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "OfferLetterData");
    XLSX.writeFile(wb, "OfferLetterData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "OfferLetterData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "OfferLetterDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert OfferLetterData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(OfferLetterData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Offer Letter</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Offer Letter</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(OfferLetterData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={OfferLetterData}>CSV</CSVLink>
                </button>
                <button
                ...
                <table className="min-w-full my-2 ">
                  <thead className="">
                    <tr className="border bg-[#fdfdfd]">
                      ...
                      <th
                        onClick={() => handleSort("desgination")}
                        className="px-5 pt-[13px] pb-[15px] text-left text-sm font-medium text-customblack tracking-wider"
                      >
                        <div className="flex items-center justify-between">
                          <div>Desgination</div>
                        ...
                        <tr key={business.id}>
                          <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                            <div className="flex items-center gap-2">
                              <p className="text-btnpurple">{business.title}</p>
                              <div className="flex items-center gap-1 text-[#00AAFF] bg-[#C9E2EF] py-1 px-2 rounded-full">
                                <AiOutlineFileText className="text-xs font-normal" />
                          ...
                          <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-1 text-[#01B81A] bg-[#C9E4CD] py-1 px-2 rounded-full">
                                <AiOutlineFileText className="text-xs font-normal" />
                                <p className="text-xs font-normal">Paid</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-5 pt-[11px] pb-[10px] whitespace-nowrap text-base font-normal text-customblack">
                            <div className="flex items-center gap-2">
                              <div className="flex items-center gap-2 cursor-pointer text-btnpurple border py-1.5 px-2 rounded-full hover:text-white hover:bg-btnpurple">
                                <AiOutlineUser className="text-sm font-normal" />
