...
const Persons = () => {
  ...
  const openModal = () => {
    ...
    const nextId = PersonsData.length + 1;
    setIsModalOpen(true);
  ...
  const handleSubmit = () => {
    // Add the new entry to PersonsData
    setPersonsData([...PersonsData, formData]);
    // Close the modal
    closeModal();
  };
  ...
  const handleSave = () => {
    // Save the edited data to the PersonsData array
    const updatedPersonsData = PersonsData.map((entry) => {
  ...
  useEffect(() => {
    ...
    const storedData = JSON.parse(localStorage.getItem("PersonsData")) || [];
    setPersonsData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever PersonsData changes
    localStorage.setItem("PersonsData", JSON.stringify(PersonsData));
  }, [PersonsData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from PersonsData
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Person List</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Person </NavLink> - List
            </div>
          </div>
