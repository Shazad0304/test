...
const StampingVisa = () => {
  ...
  const [StampingVisaData, setStampingVisaData] = useState([
    // {
    //   id: 1,
    //   title: "Anwar key",
    //   price: 3000,
  ...
  const handleSave = () => {
    // Save the edited data to the StampingVisaData array
    const updatedStampingVisaData = StampingVisaData.map((entry) => {
      if (entry.id === editedEntry.id) {
        return { ...entry, ...editedData };
      }
      return entry;
    });
    setStampingVisaData(updatedStampingVisaData);
  ...
  useEffect(() => {
    ...
    const storedData =
      JSON.parse(localStorage.getItem("StampingVisaData")) || [];
    setStampingVisaData(storedData);
  }, []);

  useEffect(() => {
    // Update localStorage whenever StampingVisaData changes
    localStorage.setItem(
      "StampingVisaData",
      JSON.stringify(StampingVisaData)
    );
  }, [StampingVisaData]);

  const handleDeleteEntry = () => {
    if (entryToDelete !== null) {
      // Remove the selected entry from StampingVisaData
      const updatedStampingVisaData = StampingVisaData.filter(
  ...
  let filteredData = [...StampingVisaData];
  if (searchQuery) {
  ...
  const handleExportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(StampingVisaData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "StampingVisaData");
    XLSX.writeFile(wb, "StampingVisaData.xlsx");
  };

  //Export PDF generatePrint
  const generatePDF = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "StampingVisaData",
  });
  const generatePrint = useReactToPrint({
    content: () => componentPDF.current,
    documentTitle: "StampingVisaDataPrint",
  });

  const handleCopyToClipboard = () => {
    // Convert StampingVisaData to a JSON string to copy it to the clipboard
    const jsonString = JSON.stringify(StampingVisaData);
      ...
      <div className="text-xl text-gray-900 font-jost font-semibold w-full ">
        ...
        <div className="container p-4 h-fit">
          <div className="flex justify-between items-center ">
            <h1 className="sm:text-xl text-sm">Stamping Visa</h1>
            <div className="text-sm text-customicongrey">
              <NavLink to="/">Stamping Visa</NavLink> - List
            </div>
          </div>
          <div className="bg-white rounded-lg my-5">
            ...
            <div className="px-6 py-4">
              <div className="buttons-export sm:flex items-center gap-1 mb-2 grid grid-cols-3">
                <CopyToClipboard text={JSON.stringify(StampingVisaData)}>
                  <button
                ...
                <button className="csv-button border bg-btnpurple text-white font-normal text-base px-4 py-1 hover:bg-white hover:text-btnpurple hover:border-btnpurple">
                  <CSVLink data={StampingVisaData}>CSV</CSVLink>
                </button>
                <button
